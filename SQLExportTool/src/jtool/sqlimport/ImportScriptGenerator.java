...
public class ImportScriptGenerator {
  static Pattern fileNamePattern = Pattern.compile(
      "\\d+[\\-_]*([A-Z]+[A-Z_0-9]*)[^A-Z0-9_]*[^\\.]*\\.([A-Z]+)", Pattern.CASE_INSENSITIVE);
  ...
  public static void generateScript(String dataDir) throws Exception {
    ...
    for (String file : files) {
      ...
      if (suffix.equalsIgnoreCase("csv")) {
        System.out
            .println("java -Xmx4096m jtool.sqlimport.ImportCSV %driver% \"%url%\" %dbuser% %dbpass% "
                + tableName + " ./csv/" + file + " UTF-8 true");
        // System.out.println("ImportCSV.imp(driver, url, user, password, \"" + tableName +
        // "\", baseFilePath + \"" + file + "\", encode, true);");
      } else if (suffix.toLowerCase().contains("xls")) {
        // System.out.println("ImportExcel.imp(driver, url, user, password, \"" + tableName +
        // "\", baseFilePath + \"" + file + "\", true);");
        System.out
            .println("java -Xmx4096m jtool.sqlimport.ImportExcel %driver% \"%url%\" %dbuser% %dbpass% "
                + tableName + " ./csv/" + file + " UTF-8 true");
      } else {
